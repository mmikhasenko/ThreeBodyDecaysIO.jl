...
function (bw::BreitWignerRhoNoSqrt)(σ::Float64)
    @unpack m, Γ, mπ, d = bw
...
function (bw::KatchaevSigma)(σ::Float64)
    mπ = 0.13956755
    mπ2 = mπ^2
...
# 
_fσ = KatchaevSigma(sP=[-0.0074, 0.9828], a=[0.1131, 0], c=[0.0337, -0.3185, -0.0942, -0.5927])
...
function build_compass_model(wave_description, m0)
    ...
    j = wave_description.S
    l = wave_description.L
    @unpack name = wave_description

    bw_ff = (j ≥ 0) ? isobarsV[j+1] : isobarsS[1-j]
